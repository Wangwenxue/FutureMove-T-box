...
uint8_t g_alternate_interface[USB_MAX_SUPPORTED_INTERFACES];
...
static uint8_t USB_Desc_Get_Descriptor(HID_HANDLE handle, uint8_t type, uint8_t str_num, uint16_t index, uint8_t **descriptor, uint32_t *size);
static uint8_t USB_Desc_Get_Interface(HID_HANDLE handle, uint8_t interface, uint8_t *alt_interface);
static uint8_t USB_Desc_Set_Interface(HID_HANDLE handle, uint8_t interface, uint8_t alt_interface);
static bool USB_Desc_Valid_Configation(HID_HANDLE handle, uint16_t config_val); 
static bool USB_Desc_Remote_Wakeup(HID_HANDLE handle);
static USB_ENDPOINTS *USB_Desc_Get_Endpoints(HID_HANDLE handle); 

static void usbd_event_handler(uint8_t event_type, void* val,void *arg); 
static uint8_t usbd_hid_handler(uint8_t request, uint16_t value, uint8_t **data, uint32_t* size, void *arg); 
...
void usbd_event_handler(uint8_t event_type, void* val, void *arg) 
{    
    UNUSED_ARGUMENT (arg)
    UNUSED_ARGUMENT (val)
...
uint8_t usbd_hid_handler (uint8_t request, uint16_t value, uint8_t **data, uint32_t* size, void *arg) 
{
